...
## Overview
This demo application shows a  implementation of a motion sensor combined with the on off button functionality.  The app is based on the Mesh Snip mesh\_sensor\_server sample which implements LE Mesh Sensor Server model and snip/mesh/mesh\_onoff\_server sample app which implements the LE Mesh Generic OnOff Server model.  The app is designed to work in concert with a Light Controller app.  The Switch will trigger the Light Controller to go on when motion is detected.  When the user pushes a button, the Switch will send the On command if the Light is Off, and the Off command if the light is On.
...
- Publishing on/off commands over LE mesh
...
4. After successful provisioning, user can use the Android MeshController/ Windows Mesh Client to configure the below parameters of the sensor.
...
- Configure Smart Switch to publish Sensor Data to the group. When the sensor is provisioned, it is configured to publish data to a group if the current group was configured, or to "all-nodes" if there were no groups.
- Modify publication to configuration to publish to "all-nodes".  Also set up the sensor to published data with period 320000msec (5 minutes). The default configuration of the sensor is to publish data with a publish period for when presence is not detected. When presence is detected the period is divided by 30. A message will be sent as soon as motion is detected and every 10 seconds while presence is being detected. (See notes below for motion sensor).
5. Wave your hand in front of the CYBT-213043-MESH / CYBLE-343072-MESH board to show some motion. (See notes below for motion sensor).
...
2. The application GATT database is located in mesh\_app\_lib as well, in file mesh\_app\_gatt.c. If you create a GATT database using Bluetooth&#174; Configurator, update the GATT database in the location mentioned above.
3. The PIR motion sensor is available only for CYBT-213043-MESH / CYBLE-343072-MESH kit. For all other kits supported by this app, only the button can be used for control.
